...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "filter_group SET sort_order = '" . (int)$data['sort_order'] . "', in_attribute = '" . (int)$data['in_attribute'] . "'");
	...
	public function editFilter($filter_group_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "filter_group SET sort_order = '" . (int)$data['sort_order'] . "', in_attribute = '" . (int)$data['in_attribute'] . "' WHERE filter_group_id = '" . (int)$filter_group_id . "'");
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				if ($filter['filter_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_id = '" . (int)$filter['filter_id'] . "', filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "', status = '" .(int)$filter['status'] . "'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "', status = '" .(int)$filter['status'] . "'");
				}
